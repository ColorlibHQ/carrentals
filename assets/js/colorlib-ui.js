/*! ColorlibUI 3.0.0 — core, owl, justified, ajaxchimp, datepicker, superfish, owl markup 2.0. Built for this theme from core + the modules it uses. */
/**
...
/* ColorlibUI module: justified — replaces justifiedGallery 3.7.0 (jQuery). A port of its
 * row algorithm: thumbnails are measured (their natural size, as the plugin's preloader
 * did), packed into rows until the next one would push the row below rowHeight, and each
 * row is scaled to fill the width. It writes what the plugin wrote: the classes
 * justified-gallery, jg-entry, entry-visible and a .caption per entry when captions are on,
 * and the same inline sizes (entry width/height/top/left, image width/height and negative
 * margins, gallery height), so justifiedGallery.css and theme CSS keep working unchanged.
 */
(function () {
  'use strict';
  var UI = window.ColorlibUI;
  ...
  var DEFAULTS = {
    ...
    triggerEvent: function (name) {
      UI.emit(this.gallery, name, { instance: this });
    }
  };
  var LAST_ROW = ['justify', 'nojustify', 'left', 'center', 'right', 'hide'];

  /* ----- jQuery's box maths, which the plugin's numbers went through ----- */

  function px(v) {
    return parseFloat(v) || 0;
  }

  function paddingBorder(cs, prop) {
    return prop === 'width' ?
  ...
  /** $(el).width() / .height(): the content size, fractional. */
  function getBox(el, prop) {
    var cs = getComputedStyle(el);
    var v = parseFloat(cs[prop]);
    var borderBox = cs.boxSizing === 'border-box';
    if (isNaN(v) && el.getClientRects().length) {
      v = prop === 'width' ? el.offsetWidth : el.offsetHeight;
      borderBox = true;
    }
    return (v || 0) - (borderBox ? paddingBorder(cs, prop) : 0);
  }

  /** $(el).width(v) / .height(v): v is the content size, so border-box adds padding+border. */
  function setBox(el, prop, v) {
    if (typeof v !== 'number' || v !== v) return;
    var cs = getComputedStyle(el);
    el.style[prop] = Math.max(0, v + (cs.boxSizing === 'border-box' ? paddingBorder(cs, prop) : 0)) + 'px';
  }

  /** "> img, > a > img" relative to el (jQuery .find() with a leading combinator). */
  function scoped(selector) {
    return selector.split(',').map(function (part) { return ':scope ' + part.trim(); }).join(', ');
  }
  ...
  function hasScrollbar() {
    return getBox(document.body, 'height') > document.documentElement.clientHeight;
  }

  function data(entry) {
    return entry.__jg || (entry.__jg = {});
  }

  function Gallery(el, settings) {
    this.gallery = el;
    ...
    this.spinner.innerHTML = '<span></span><span></span><span></span>';
    this.spinnerPhase = 0;
    this.spinnerId = null;
    this.scrollBarOn = false;
    this.checkWidthIntervalId = null;
    this.galleryWidth = getBox(el, 'width');
    this.tempHeight = 0;
  }

  var proto = Gallery.prototype;

  proto.checkSettings = function () {
    var s = this.settings;
    ['rowHeight', 'margins', 'border', 'justifyThreshold', 'refreshTime', 'refreshSensitivity'].forEach(function (k) {
      s[k] = parseFloat(s[k]);
      if (isNaN(s[k])) s[k] = DEFAULTS[k];
    });
    if (LAST_ROW.indexOf(s.lastRow) === -1) s.lastRow = DEFAULTS.lastRow;
  };

  proto.retrieveMaxRowHeight = function () {
    var max = this.settings.maxRowHeight;
    var rowHeight = this.settings.rowHeight;
    var h;
    if (typeof max === 'string') {
      h = /^[0-9]+%$/.test(max) ? rowHeight * parseFloat(max) / 100 : parseFloat(max);
    ...
    return h < rowHeight ? rowHeight : h;
  };

  proto.updateSettings = function (options) {
    this.settings = UI.extend({}, this.settings, options);
    this.checkSettings();
  ...
  proto.trigger = function (name) {
    this.settings.triggerEvent.call(this, name);
  };

  proto.imgFromEntry = function (entry) {
    return entry.querySelector(scoped(this.settings.imgSelector));
  };

  proto.captionFromEntry = function (entry) {
    return entry.querySelector(':scope > .caption');
  };

  /* ----- showing entries ----- */

  proto.displayEntry = function (entry, x, y, imgWidth, imgHeight, rowHeight) {
    var self = this;
    setBox(entry, 'width', imgWidth);
    setBox(entry, 'height', rowHeight);
    ...
    if (img) {
      ...
      // The fade-in lives in justifiedGallery.css; reduced motion shows the image at once.
      if (UI.reducedMotion) img.style.transition = 'none';
      var src = img.getAttribute('src');
      var newSrc = this.settings.thumbnailPath ?
        this.settings.thumbnailPath(src, imgWidth, imgHeight, img) : src;
      var swap = function () {
        if (src === newSrc) return;
        img.addEventListener('error', function () {
          img.setAttribute('src', data(img).originalSrc);
        }, { once: true });
        img.setAttribute('src', newSrc);
      };
      if (data(entry).loaded === 'skipped') {
        this.onImageEvent(src, function () {
          self.showImg(entry, swap);
          data(entry).loaded = true;
        });
      } else {
        this.showImg(entry, swap);
      }
    } else {
      this.showImg(entry);
    }
  ...
  proto.showImg = function (entry, callback) {
    entry.classList.add('entry-visible');
    if (callback) callback();
  };

  proto.displayEntryCaption = function (entry) {
    var img = this.imgFromEntry(entry);
    if (!img || !this.settings.captions) {
    ...
    if (!caption) {
      ...
      if (!text) text = entry.getAttribute('title');
      if (text) {
        caption = document.createElement('div');
        caption.className = 'caption';
        caption.textContent = text;
        entry.appendChild(caption);
        data(entry).createdCaption = true;
      }
    }
    if (caption) {
      if (UI.reducedMotion) caption.style.transition = 'none';
      this.addCaptionEventsHandlers(entry);
    }
  };

  proto.addCaptionEventsHandlers = function (entry) {
    var d = data(entry);
    var self = this;
    if (d.captionHandlers) return;
    d.captionHandlers = {
      mouseenter: function () {
        var c = self.captionFromEntry(entry);
        if (!c) return;
        c.classList.add('caption-visible');
        c.classList.remove('caption-hidden');
      },
      mouseleave: function () {
        var c = self.captionFromEntry(entry);
        if (!c) return;
    ...
    entry.addEventListener('mouseenter', d.captionHandlers.mouseenter);
    entry.addEventListener('mouseleave', d.captionHandlers.mouseleave);
  };

  proto.removeCaptionEventsHandlers = function (entry) {
    var d = data(entry);
    if (!d.captionHandlers) return;
    entry.removeEventListener('mouseenter', d.captionHandlers.mouseenter);
    entry.removeEventListener('mouseleave', d.captionHandlers.mouseleave);
    delete d.captionHandlers;
  };

  /* ----- rows ----- */

  proto.clearBuildingRow = function () {
    this.buildingRow.entriesBuff = [];
    this.buildingRow.aspectRatio = 0;
    this.buildingRow.width = 0;
  };

  /** Size the row's entries. Returns whether the row is justified (-1: hidden last row). */
  proto.prepareBuildingRow = function (isLastRow) {
    var s = this.settings;
    var row = this.buildingRow;
    var justify = true;
    var minHeight = 0;
    var availableWidth = this.galleryWidth - 2 * this.border - (row.entriesBuff.length - 1) * s.margins;
    var rowHeight = availableWidth / row.aspectRatio;
    var defaultRowHeight = s.rowHeight;
    var justifiable = row.width / availableWidth > s.justifyThreshold;
    var i, entry, newImgW, newImgH;

    if (isLastRow && s.lastRow === 'hide' && !justifiable) {
      row.entriesBuff.forEach(function (e) { e.classList.remove('entry-visible'); });
      return -1;
    }

    if (isLastRow && !justifiable && s.lastRow !== 'justify' && s.lastRow !== 'hide') {
      justify = false;
      if (this.rows > 0) {
        // Keep the unfinished last row at the average height of the rows above it.
        defaultRowHeight = (this.offY - this.border - s.margins * this.rows) / this.rows;
        justify = defaultRowHeight * row.aspectRatio / availableWidth > s.justifyThreshold;
      }
    }

    for (i = 0; i < row.entriesBuff.length; i++) {
      entry = row.entriesBuff[i];
      var ratio = data(entry).width / data(entry).height;
      if (justify) {
        // The last image takes whatever rounding left over, so the row ends flush.
        newImgW = i === row.entriesBuff.length - 1 ? availableWidth : rowHeight * ratio;
        newImgH = rowHeight;
      } else {
        newImgW = defaultRowHeight * ratio;
        newImgH = defaultRowHeight;
      }
      availableWidth -= Math.round(newImgW);
      data(entry).jwidth = Math.round(newImgW);
      data(entry).jheight = Math.ceil(newImgH);
      if (i === 0 || newImgH < minHeight) minHeight = newImgH;
    }
    row.height = minHeight;
    return justify;
  };

  proto.flushRow = function (isLastRow) {
    var s = this.settings;
    var row = this.buildingRow;
    var offX = this.border;
    var justified = this.prepareBuildingRow(isLastRow);
    var i, entry;

    if (isLastRow && s.lastRow === 'hide' && justified === -1) {
      this.clearBuildingRow();
      return;
    }
    if (this.maxRowHeight && this.maxRowHeight < row.height) row.height = this.maxRowHeight;

    if (isLastRow && (s.lastRow === 'center' || s.lastRow === 'right')) {
      var remaining = this.galleryWidth - 2 * this.border - (row.entriesBuff.length - 1) * s.margins;
      for (i = 0; i < row.entriesBuff.length; i++) remaining -= data(row.entriesBuff[i]).jwidth;
      offX += s.lastRow === 'center' ? remaining / 2 : remaining;
    }

    var last = row.entriesBuff.length - 1;
    for (i = 0; i <= last; i++) {
      entry = row.entriesBuff[s.rtl ? last - i : i];
      this.displayEntry(entry, offX, this.offY, data(entry).jwidth, data(entry).jheight, row.height);
      offX += data(entry).jwidth + s.margins;
    }

    this.galleryHeightToSet = this.offY + row.height + this.border;
    this.setGalleryTempHeight(this.galleryHeightToSet + this.getSpinnerHeight());

    if (!isLastRow || (row.height <= s.rowHeight && justified)) {
      this.offY += row.height + s.margins;
      this.rows += 1;
      this.clearBuildingRow();
      this.trigger('jg.rowflush');
    }
  };

  /* ----- gallery height and the loading spinner ----- */

  proto.rememberGalleryHeight = function () {
    this.tempHeight = getBox(this.gallery, 'height');
    setBox(this.gallery, 'height', this.tempHeight);
  };

  proto.setGalleryTempHeight = function (h) {
    this.tempHeight = Math.max(h, this.tempHeight);
    setBox(this.gallery, 'height', this.tempHeight);
  };

  proto.setGalleryFinalHeight = function (h) {
    if (typeof h !== 'number') return;
    this.tempHeight = h;
    setBox(this.gallery, 'height', h);
  };

  proto.isSpinnerActive = function () {
    return this.spinnerId !== null;
  };

  /** $spinner.innerHeight(): 0 while detached. */
  proto.getSpinnerHeight = function () {
    var el = this.spinner;
    if (!el.parentNode) return 0;
    var cs = getComputedStyle(el);
    var h = px(cs.height);
    return cs.boxSizing === 'border-box' ?
      h - px(cs.borderTopWidth) - px(cs.borderBottomWidth) :
      h + px(cs.paddingTop) + px(cs.paddingBottom);
  };

  proto.startLoadingSpinnerAnimation = function () {
    var self = this;
    var spans = this.spinner.children;
    clearInterval(this.spinnerId);
    this.gallery.appendChild(this.spinner);
    this.setGalleryTempHeight(this.offY + this.buildingRow.height + this.getSpinnerHeight());
    // Three dots fading in, then out, one after another (150 ms steps, as the plugin).
    this.spinnerId = setInterval(function () {
      var n = spans.length;
      var p = self.spinnerPhase;
      var dot = spans[p < n ? p : p - n];
      dot.style.transition = UI.reducedMotion ? 'none' : 'opacity 150ms';
      dot.style.opacity = p < n ? '1' : '0';
      self.spinnerPhase = (p + 1) % (2 * n);
    }, 150);
  };

  proto.stopLoadingSpinnerAnimation = function () {
    clearInterval(this.spinnerId);
    this.spinnerId = null;
    this.setGalleryTempHeight(getBox(this.gallery, 'height') - this.getSpinnerHeight());
    if (this.spinner.parentNode) this.spinner.parentNode.removeChild(this.spinner);
  };

  /**
   * The plugin polled the gallery width every refreshTime ms rather than listening for
   * window resize, so it also caught containers that change width on their own. Kept, with
   * its guard: a width change caused only by the page scrollbar appearing or going is
   * recorded without a re-layout (otherwise a layout that adds a scrollbar loops).
   */
  proto.checkWidth = function () {
    var self = this;
    if (this.checkWidthIntervalId !== null) return;
    this.checkWidthIntervalId = setInterval(function () {
      if (!isVisible(self.gallery)) return;
      var width = getBox(self.gallery, 'width');
      if (hasScrollbar() === self.scrollBarOn) {
        if (Math.abs(width - self.galleryWidth) > self.settings.refreshSensitivity) {
          self.galleryWidth = width;
          self.rewind();
          self.rememberGalleryHeight();
          self.startImgAnalyzer(true);
        }
      } else {
        self.scrollBarOn = hasScrollbar();
        self.galleryWidth = width;
      }
  ...
  proto.updateEntries = function (norewind) {
    var s = this.settings;
    var list = [];
    var el;
    if (norewind && this.lastFetchedEntry) {
      for (el = this.lastFetchedEntry.nextElementSibling; el; el = el.nextElementSibling) {
        if (el.matches(s.selector)) list.push(el);
      }
    } else {
      this.entries = [];
      list = Array.prototype.filter.call(this.gallery.children, function (c) { return c.matches(s.selector); });
    }
    if (list.length) {
      if (typeof s.sort === 'function') {
        list.sort(s.sort);
        this.insertToGallery(list);
      } else if (s.randomize) {
      ...
      this.lastFetchedEntry = list[list.length - 1];
      list = s.filter ? this.filterArray(list) : this.resetFilters(list);
    }
    this.entries = this.entries.concat(list);
    return true;
  };

  proto.insertToGallery = function (list) {
    var g = this.gallery;
    list.forEach(function (el) { g.appendChild(el); });
  };

  proto.resetFilters = function (list) {
    list.forEach(function (el) { el.classList.remove('jg-filtered'); });
    return list;
  };

  proto.filterArray = function (list) {
    var filter = this.settings.filter;
    var kept = typeof filter === 'function' ? list.filter(filter) :
      list.filter(function (el) { return el.matches(filter); });
    list.forEach(function (el) {
      if (kept.indexOf(el) === -1) {
        el.classList.add('jg-filtered');
        el.classList.remove('jg-visible');
      } else {
        el.classList.remove('jg-filtered');
      }
    });
    return kept;
  };

  /** Load a thumbnail off-DOM to learn its natural size, as the plugin's preloader did. */
  proto.onImageEvent = function (src, onLoad, onError) {
    if (!onLoad && !onError) return;
    var image = new Image();
    image.addEventListener('load', function () { if (onLoad) onLoad(image); });
    image.addEventListener('error', function () { if (onError) onError(image); });
    image.src = src;
  };
  ...
  proto.init = function () {
    ...
    this.entries.forEach(function (entry) {
      var d = data(entry);
      var img = self.imgFromEntry(entry);
      entry.classList.add('jg-entry');
      if (d.loaded === true || d.loaded === 'skipped') return;
      if (s.rel !== null) entry.setAttribute('rel', s.rel);
      if (s.target !== null) entry.setAttribute('target', s.target);
      if (img) {
        var src = img.hasAttribute('data-safe-src') ? img.getAttribute('data-safe-src') : img.getAttribute('src');
        data(img).originalSrc = src;
        if (src !== null) img.setAttribute('src', src);
        if (s.waitThumbnailsLoad === false) {
          ...
          if (!isNaN(w) && !isNaN(h)) {
            ...
            d.loaded = 'skipped';
            skipped = true;
            self.startImgAnalyzer(false);
            return;
          }
        }
        d.loaded = false;
        waiting = true;
        if (!self.isSpinnerActive()) self.startLoadingSpinnerAnimation();
        self.onImageEvent(src, function (image) {
          d.width = image.width;
        ...
        // No image: size the entry by itself (the plugin OR-ed these, bitwise).
        var cs = getComputedStyle(entry);
        d.loaded = true;
        d.width = getBox(entry, 'width') | parseFloat(cs.width) | 1;
        d.height = getBox(entry, 'height') | parseFloat(cs.height) | 1;
      }
    });
    if (!waiting && !skipped) this.startImgAnalyzer(false);
    this.checkWidth();
  };

  /** Fill rows with every entry measured so far, in order; stop at one still loading. */
  proto.analyzeImages = function (isForResize) {
    var s = this.settings;
    var row = this.buildingRow;
    for (var i = this.lastAnalyzedIndex + 1; i < this.entries.length; i++) {
      var d = data(this.entries[i]);
      if (d.loaded === true || d.loaded === 'skipped') {
        var availableWidth = this.galleryWidth - 2 * this.border - (row.entriesBuff.length - 1) * s.margins;
        var ratio = d.width / d.height;
        if (availableWidth / (row.aspectRatio + ratio) < s.rowHeight) {
          this.flushRow(false);
          // Give the page a frame every two rows, as the plugin did.
          if (++this.yieldFlushed >= 2) {
            this.startImgAnalyzer(isForResize);
            return;
          }
        }
        row.entriesBuff.push(this.entries[i]);
        row.aspectRatio += ratio;
        row.width += ratio * s.rowHeight;
        this.lastAnalyzedIndex = i;
      } else if (d.loaded !== 'error') {
        return;
      }
    }
    if (row.entriesBuff.length > 0) this.flushRow(true);
    if (this.isSpinnerActive()) this.stopLoadingSpinnerAnimation();
    this.stopImgAnalyzerStarter();
    this.trigger(isForResize ? 'jg.resize' : 'jg.complete');
    this.setGalleryFinalHeight(this.galleryHeightToSet);
  };

  proto.stopImgAnalyzerStarter = function () {
    this.yieldFlushed = 0;
  ...
  proto.startImgAnalyzer = function (isForResize) {
    var self = this;
    this.stopImgAnalyzerStarter();
    this.imgAnalyzerTimeout = setTimeout(function () {
      self.analyzeImages(isForResize);
    }, 0);
  };
  ...
  proto.destroy = function () {
    ...
    this.stopImgAnalyzerStarter();
    this.entries.forEach(function (entry) {
      entry.style.width = entry.style.height = entry.style.top = entry.style.left = '';
      data(entry).loaded = undefined;
      entry.classList.remove('jg-entry');
      var img = self.imgFromEntry(entry);
      if (img) {
        img.style.width = img.style.height = img.style.marginLeft = img.style.marginTop = '';
        if (data(img).originalSrc != null) img.setAttribute('src', data(img).originalSrc);
      }
      self.removeCaptionEventsHandlers(entry);
      var caption = self.captionFromEntry(entry);
      if (data(entry).createdCaption) {
        data(entry).createdCaption = undefined;
    ...
    this.gallery.removeAttribute('data-cl-justified');
    delete this.gallery.__clJustified;
  };

  /** $(target).justifiedGallery(options | 'norewind' | 'destroy') */
  UI.justifiedGallery = function (target, options) {
    var list = [];
    UI.each(target, function (el) {
      el.classList.add('justified-gallery');
      var g = el.__clJustified;
      if (!g) {
        if (options != null && typeof options !== 'object') return;
        el.setAttribute('data-cl-justified', '1');
        g = el.__clJustified = new Gallery(el, UI.extend({}, DEFAULTS, options));
      } else if (options === 'destroy') {
        g.destroy();
        return;
      } else if (options !== 'norewind') {
        // A second call with options re-lays the gallery out with them.
        g.updateSettings(options);
        g.rewind();
      }
      if (g.updateEntries(options === 'norewind')) g.init();
      list.push(g);
    });
    return list;
  };
