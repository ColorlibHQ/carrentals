...
jQuery( document ).ready(
	function () {
			...
			jQuery( '.carrentals-elementor-notice-buttons > a' ).on(
				'click', function() {
					...
					jQuery.ajax(
						{
							url: carrentalsElementorNotice.ajaxurl,
							data: {
								reply: reply,
								nonce: carrentalsElementorNotice.nonce,
							...
							success: function () {
								...
								if ( reply === 'yes' ) {
									...
									jQuery( '.carrentals-disable-elementor-styling' ).fadeOut( 500, function() { jQuery( this ).remove(); } );
								}
							}
						}
					);
