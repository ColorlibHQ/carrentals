...
function carrentals_sl_enqueue_scripts() {
	...
	wp_localize_script( 'carrentals-post-likes', 'simpleLikes', array(
		...
		'like'    => esc_html__( 'Like', 'carrentals' ),
		'unlike'  => esc_html__( 'Unlike', 'carrentals' )
	) );
}
...
function carrentals_process_simple_like() {
	...
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'carrentals' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'carrentals' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'carrentals' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'carrentals' );
...
function carrentals_show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'carrentals' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'carrentals' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
